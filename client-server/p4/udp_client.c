...
int main(){
    ...
    for(int i=5000;i<6000;i++){
        ...
        if(sendto(udp_socket,&msg,sizeof(msg),0,(struct sockaddr*)&server,sizeof(server))!=-1){
            printf("Port: %d\n",i);
            connected=1;
            break;
        }
    ...
    if(recvfrom(udp_socket,&msg,sizeof(msg),0,(struct sockaddr*)&server,&srv_size)==-1){
    ...
    pthread_create(&sender,NULL,_send,&udp_socket);
    pthread_create(&receiver,NULL,receive,&udp_socket);
